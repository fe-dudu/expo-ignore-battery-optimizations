...
const withBatteryOptimizationPermission: ConfigPlugin<void> = (config) => {
  return AndroidConfig.Permissions.withPermissions(config, [BATTERY_OPT_PERMISSION]);
};

export default createRunOncePlugin(withBatteryOptimizationPermission, pkg.name, pkg.version);
